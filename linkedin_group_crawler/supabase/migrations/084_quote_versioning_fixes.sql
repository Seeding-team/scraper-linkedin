-- Fix 2 bug that qua test thuc te tren quote_create_version() (migration 082):
--
-- 1) "DELETE FROM tmp_quote_item_map;" (khong WHERE) bi DB tu chan boi 1 rule
--    an toan co san ("DELETE requires a WHERE clause") - va cau lenh nay VON
--    DA THUA: bang TEMP ... ON COMMIT DROP luon bi xoa het khi transaction
--    (chinh la lan goi RPC do) commit xong, nen lan goi SAU luon thay bang
--    trong tinh (CREATE TEMP TABLE IF NOT EXISTS tao lai moi). Bo han dong
--    DELETE nay, khong thay the bang gi khac.
--
-- 2) Bam "Tao phien ban moi" THANG vao 1 bao gia dang la draft (chua duyet)
--    truoc day KHONG bi chan o RPC - nhanh "da co ban nhap trong chuoi" tim
--    thay CHINH bao gia draft vua bam roi coi no la "ban nhap co san", tra ve
--    created=false thay vi loi - sai voi yeu cau nghiep vu "khong the tao
--    phien ban tu 1 ban nhap" (test case bat buoc). Frontend da chan bang UI
--    (nut chi hien khi status='approved') nhung backend PHAI tu chan that su,
--    khong chi dua vao FE an nut (dung nguyen tac da ap dung cho moi endpoint
--    khac trong app nay). Fix: kiem tra status cua CHINH bao gia duoc bam
--    NGAY SAU KHI khoa chuoi - la 'draft' thi raise loi rieng
--    'quote_is_draft', truoc khi cham toi nhanh "da co ban nhap khac trong
--    chuoi" (nhanh do van giu nguyen, chi ap dung khi bao gia duoc bam KHONG
--    phai la draft - vd bam vao 1 ban DA DUYET nhung chuoi da co 1 draft khac
--    moi hon, van redirect dung nhu cu).
...
BEGIN
    ...
    -- Bam thang vao 1 ban DANG LA DRAFT -> chan het, khong redirect/tao gi.
    IF v_clicked_status = 'draft' THEN
        RAISE EXCEPTION 'quote_is_draft';
    END IF;

    -- Da co ban nhap KHAC trong chuoi -> tra ve luon ban do, KHONG tao them.
    ...
    -- KHONG can DELETE truoc CREATE - bang TEMP ... ON COMMIT DROP luon la
    -- bang moi/rong o dau moi lan goi ham (transaction truoc do da drop het).
