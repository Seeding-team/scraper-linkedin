-- Migration 128: Dạy 2 RPC cũ (fn_bulk_save_zalo_messages, fn_get_zalo_conversation_messages)
...
DECLARE
    v_saved_count INT := 0;
BEGIN
    -- 1. Bulk Upsert zalo_groups (không đổi so với migration 015)
    ...
    -- 2. Bulk Upsert zalo_messages — THÊM ts/raw_content/mentions/cli_msg_id/msg_kind
    IF p_messages IS NOT NULL AND jsonb_array_length(p_messages) > 0 THEN
        INSERT INTO public.zalo_messages (
            ...
            ts, raw_content, mentions, cli_msg_id, msg_kind
        )
        SELECT
            p_user_id,
            (m->>'group_id')::TEXT,
