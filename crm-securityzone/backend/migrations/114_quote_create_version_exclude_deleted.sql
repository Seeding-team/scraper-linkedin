-- BUG THAT DA GAP THAT SU ("mới xóa V2 nha mà còn v1 mà nó báo v là sao" -
-- QA thuc te: xoa mem 1 ban nhap V2 (soft-delete, quote_soft_delete() CHI set
-- deleted_at, KHONG doi status - van la 'draft'), sau do bam "Tạo phiên bản
-- mới" tu V1 (da duyet) thi nhan alert "Không tìm thấy báo giá." thay vi tao
-- V2 moi):
-- quote_create_version() (moi lan dinh nghia lai tu 082 - 083/084/106/112/113)
-- tim "ban nhap co san trong chuoi" bang:
--   SELECT * INTO v_existing_draft FROM public.quotes
--       WHERE version_chain_id = v_chain_id AND status = 'draft'
--       ORDER BY version_number DESC LIMIT 1;
-- KHONG CO dieu kien loai tru deleted_at - nen van tim THAY dung ban V2 VUA
-- XOA MEM o tren (status van la 'draft', chi deleted_at khac NULL), roi tra
-- ve id cua no nhu "da co san, redirect sang do thay vi tao moi". Frontend
-- (create_quote_version() python) sau do goi get_quote(id do) - ham nay MAC
-- DINH loc deleted_at IS NULL (dung y "xoa mem khong con la quote dang hoat
-- dong" - xem supabase_quote_service.py) nen KHONG tim thay, nem
-- QuoteNotFoundError -> dung 1 chu "Không tìm thấy báo giá." nguoi dung thay.
--
-- Sua CHI 1 dieu kien: them "AND deleted_at IS NULL" vao ca 2 truy van tim
-- ban ghi trong chuoi (draft co san VA ban goc da duyet dung lam nguon) -
-- dam bao khong bao gio "tim thay" 1 ban ghi da bi xoa mem, giu nguyen 100%
-- logic con lai.
