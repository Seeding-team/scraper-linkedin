-- Contact 360 - buoc 1: canonical Deal<->Contact relation.
--
-- AUDIT (xac minh that tren schema, khong doan): `customer_leads` (Deal)
-- hien KHONG co cot nao tro toi `crm_contacts` - Deal chi biet Customer
-- (customer_id). Khong co bang trung gian nhieu-nhieu nao (deal_contacts/
-- customer_lead_contacts/...) da ton tai. Business model hien tai (theo yeu
-- cau) la "1 nguoi lien he chinh moi Deal" - dung dung mo hinh don gian nhat
-- healthy voi du lieu that: 1 cot FK, khong bang trung gian thua.
--
-- Fix: them `customer_leads.primary_contact_id` (nullable, FK toi
-- crm_contacts, ON DELETE SET NULL - xoa Contact khong duoc lam mat ca Deal).
-- Cap nhat `crm_convert_lead` (migration 133) de tu dong gan cot nay bang
-- dung Contact vua tao/duoc chon trong luong convert - KHONG doan/backfill
-- nguoc cho Deal cu (xem bao cao backfill rieng, khong lam trong migration
-- nay).

ALTER TABLE public.customer_leads
    ADD COLUMN IF NOT EXISTS primary_contact_id UUID REFERENCES public.crm_contacts(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_customer_leads_primary_contact_id
    ON public.customer_leads(primary_contact_id);


-- Cap nhat crm_convert_lead: y het migration 133, chi them 1 dong gan
-- primary_contact_id = v_contact_id vao INSERT customer_leads. Khong doi
-- logic nao khac (dedup theo instance, contact.instance, idempotency,
-- provenance cua Lead deu giu nguyen y het migration 133).
...
BEGIN
    ...
    -- crm_leads. FIX (Contact 360): gan primary_contact_id = v_contact_id
    -- (co the NULL neu Lead nay khong co Contact - Deal van tao binh thuong,
    -- Contact 360 se hien "Chua co co hoi lien ket" cho cac Deal loai nay,
    -- KHONG doan/gan bua).
