...
def auth_list_workspaces(request: Request) -> BaseResponse:
    """Danh sách workspace (brand) đang cấu hình + workspace hiện tại. Không
    lộ thông tin nhạy cảm (chỉ tên brand + URL công khai) nên không cần auth —
    frontend dùng để hiện switcher (chỉ render UI khi role=admin, kiểm tra
    quyền thật ở /workspace-handoff bên dưới)."""
