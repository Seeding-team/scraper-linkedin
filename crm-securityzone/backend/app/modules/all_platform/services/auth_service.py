...
def _check_home_instance_redirect(user: dict) -> Optional[dict]:
    ...
    KHONG nam trong danh sach allowed_instances (danh sach workspace admin cho
    phep truy cap, migration 005 — gan qua "Quan ly thanh vien") -> tra ve tin
    hieu redirect ve workspace DAU TIEN duoc phep, thay vi dang nhap thang.
    Admin luon duoc bo qua (dang nhap truc tiep duoc o moi site, dung switcher
    rieng de chuyen qua lai). allowed_instances rong/NULL -> fallback ve
    home_instance (tai khoan tao truoc migration 005, chua duoc admin gan gi)
    -> van rong/NULL nua thi khong bi rang buoc gi (giu hanh vi cu)."""
