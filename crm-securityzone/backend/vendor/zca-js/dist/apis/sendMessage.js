...
export const sendMessageFactory = apiFactory()((api, ctx, utils) => {
    ...
    async function handleMessage({ msg, styles, urgency, mentions, quote, ttl, clientId }, threadId, type) {
        if (!msg || msg.length == 0)
    ...
    async function handleAttachment({ msg, attachments, mentions, quote, ttl, urgency, clientId }, threadId, type) {
        if (!attachments)
        ...
        let baseClientId = clientId || Date.now();
                ...
                case "image": {
                    data = {
                        ...
                        params: {
                            ...
                            clientId: (baseClientId++).toString(),
                            desc: msg,
    ...
    return async function sendMessage(message, threadId, type = ThreadType.User) {
        ...
        const { quote, ttl, styles, urgency, clientId } = message;
        ...
        if (attachments && attachments.length > 0) {
            ...
            const handledData = await handleAttachment({ msg, mentions, attachments, quote, ttl, styles, urgency, clientId }, threadId, type);
