...
const getCMRecentFactory = utils.apiFactory()((api, ctx, utils) => {
    ...
    return async function getCMRecent(count = 50, lastTime = 0) {
        ...
        if (!encryptedParams)
            throw new ZaloApiError.ZaloApiError("Failed to encrypt CM params");
        ...
        throw lastError instanceof Error
            ? lastError
            : new ZaloApiError.ZaloApiError(`getCMRecent failed after ${RETRY_DELAYS.length + 1} attempts`);
    };
});

exports.getCMRecentFactory = getCMRecentFactory;
