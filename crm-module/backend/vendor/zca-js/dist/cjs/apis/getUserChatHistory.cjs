...
const getUserChatHistoryFactory = utils.apiFactory()((api, ctx, utils) => {
    ...
    return async function getUserChatHistory(userId, count = 50, globalMsgId) {
        ...
        return utils.resolve(response, (result) => {
            ...
            for (let i = 0; i < data.msgs.length; i++) {
                data.msgs[i] = new Message.UserMessage(ctx.uid, data.msgs[i]);
            }
            return data;
        });
    };
...
exports.getUserChatHistoryFactory = getUserChatHistoryFactory;
