 ...
 */
// ========== Service ==========
class ZaloSyncService extends EventEmitter {
    constructor(options = {}) {
        super();
        this.api = null;
        this.startTime = 0;
        ...
        this.zalo = new zalo.Zalo(options);
    }
    // ========== Login Methods ==========
    /**
    ...
    async startSync() {
        ...
        listener.on("message", (message) => {
            ...
            if (message.type === Enum.ThreadType.User) {
    ...
    async crawlUserChatHistory(userId, options = {}) {
        ...
        try {
            ...
            if (this.isListening) {
                ...
                while (allMessages.length < maxMessages && hasMore) {
                    const wsMsgs = await this.api.listener.requestOldMessagesAsync(Enum.ThreadType.User, lastMsgId, 15000);
     ...
     */
    async sendMessage(content, threadId, type = Enum.ThreadType.User) {
        if (!this.api)
            throw new Error("Not logged in");
    ...
    async fetchMissedMessages() {
        ...
        try {
            ...
            const userMsgs = await this.api.listener.requestOldMessagesAsync(Enum.ThreadType.User, null, 15000);
            if (userMsgs.length > 0) {
                this.emit("missed_messages", userMsgs, Enum.ThreadType.User);
            }
        }
        catch (_a) {
        ...
        try {
            ...
            const groupMsgs = await this.api.listener.requestOldMessagesAsync(Enum.ThreadType.Group, null, 15000);
            if (groupMsgs.length > 0) {
                this.emit("missed_messages", groupMsgs, Enum.ThreadType.Group);
            }
        }
        catch (_b) {
...
exports.ZaloSyncService = ZaloSyncService;
