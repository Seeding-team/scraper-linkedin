-- BUG THAT DA GAP (2 loi rieng biet, cung nam trong crm_convert_lead - migration
-- 120 da them p_instance/v_instance nhung bo sot 2 cho sau):
--
-- 1) Dedup khach hang theo email/phone/tax_code/website/ten (nhanh 309-373)
--    KHONG loc theo instance -> mot Lead convert o tenant A co the "tim thay
--    trung" va gan vao Customer cua tenant B chi vi trung so dien thoai/email
--    tren cung 1 DB dung chung. Da xac minh that: convert Lead tren
--    crm-securityzone bi gan nham vao 1 Customer that cua crm-cloudgate.
--    Fix: them dieu kien AND instance = v_instance vao WHERE.
--
-- 2) INSERT INTO crm_contacts (nhanh 383-401) KHONG liet ke cot `instance`
--    trong danh sach cot -> Postgres tu dien DEFAULT cua cot (la 'markee')
--    cho MOI contact tao qua RPC nay, bat ke tenant nao goi. Hau qua: contact
--    tao tu Lead convert tren crm-securityzone/crm-cloudgate bi gan instance=
--    'markee', nen GET .../contacts (loc .eq("instance", settings.crm_instance))
--    tren chinh tenant do luon tra ve rong - contact "bien mat" vinh vien.
--    Fix: them `instance` vao danh sach cot INSERT, dung v_instance (KHONG dua
--    vao DEFAULT 'markee').
--
-- Khong doi bat ky logic nao khac: Customer=cong ty, Contact=nguoi, cac field
-- provenance cua Lead (converted_customer_id/converted_contact_id/converted_deal_id/
-- converted_by/converted_at), idempotency, va dedup CUNG tenant giu nguyen y het
-- (v_instance cua ban ghi cung tenant se vi khop dieu kien AND instance = v_instance).
...
BEGIN
    ...
    ELSIF p_customer IS NOT NULL THEN
        ...
        PERFORM pg_advisory_xact_lock(
            hashtextextended(
                'crm_customer|' || v_instance || '|' || COALESCE(v_email, '') || '|' || COALESCE(v_phone, '') || '|' ||
                COALESCE(v_tax_code, '') || '|' || COALESCE(v_website_domain, ''),
                0
            )
        );

        -- FIX (bug 1): dedup lookup phai loc theo instance - truoc day thieu
        -- dieu kien nay nen co the "tim thay trung" va gan nham vao Customer
        -- cua MOT TENANT KHAC tren cung DB dung chung.
    ...
    ELSIF p_contact IS NOT NULL AND p_contact <> 'null'::jsonb AND p_contact <> '{}'::jsonb THEN
        -- FIX (bug 2): phai liet ke `instance` va dua tri v_instance - truoc
        -- day thieu cot nay nen Postgres tu dien DEFAULT 'markee' cho moi
        -- contact, bat ke tenant nao goi RPC.
